...
class ModbusMasterClientWidget:
    ...
    def retrieve_data(self):
        ...
        chunk_size = int(1)
        for address in range(0 ,count, chunk_size):

            try:
                result = self.modbus_client.client.read_holding_registers(address, chunk_size, unit=unit)
                if not result.isError():
                    print(f"Register at address {address} is readable. Value: {result.registers[0]}")
                    # Clear the table
